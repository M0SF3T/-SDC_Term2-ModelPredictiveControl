...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  for (unsigned int i = delta_start; i < a_start; i++) {
    vars_lowerbound[i] = -0.436332; // try -1.0 to 1.0
  ...
  for (unsigned int i = a_start; i < n_vars; i++) {
    vars_lowerbound[i] = -1.0; // try + / - 0.435332 * LF
  ...
  //Pushes back into our result vector the first result for the actuators
  result.push_back(solution.x[delta_start]);
  result.push_back(solution.x[a_start]);

  // Fetches x and y positionns for predicted postions points
  for (unsigned int t = 0; t <N-1; t++)
  {
